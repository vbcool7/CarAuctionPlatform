            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-center gap-3">
                ...
                <div className="relative w-30 h-30 md:w-32 md:h-32">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        ...
                        <span className="text-[7px] sm:text-[8px] font-bold text-slate-900 uppercase tracking-wider">
                            {centerLabel}
                        </span>
                    </div>
                </div>
                    ...
                    {segments.map(item => (
                        <div
                            ...
                            <span className="text-xs sm:text-sm font-medium text-slate-600 whitespace-nowrap">
                                {prefix}{item.value.toLocaleString()}{showPercentage && ` (${total > 0 ? Math.round((item.value / total) * 100) : 0}%)`}
                            </span>
                        </div>
                    ))}
                </div>
