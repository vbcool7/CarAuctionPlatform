...
function RefundsList({ setCurrentPage }) {
      ...
      <div className="overflow-x-auto border border-gray-200 rounded-lg">
        <table className="w-full text-left table-fixed">
          ...
          <tbody className="divide-y divide-amber-50">
            {filteredRefunds.map((payout, index) => (
              <tr
                key={payout.refundId || index}
              ...
              >
                ...
                <td className="px-6 py-4 text-[13px] text-slate-700 leading-tight">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 rounded-md flex items-center justify-center bg-slate-100 text-slate-600 shrink-0">
                      ...
                      {!["Bank Transfer", "Wire Transfer", "PayPal", "Wise"].includes(payout.method) && (
                ...
                <td className="px-6 py-4 text-[13px] leading-tight">
                  <div className="font-semibold text-slate-800">{payout.reason || '---'}</div>
                </td>

                {/* status */}
                <td className="px-6 py-4">
                  <span
                    className={`px-2 py-1 rounded text-[11px] font-medium border ${payout.status === 'Completed'
