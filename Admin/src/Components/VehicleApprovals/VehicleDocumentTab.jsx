...
function DocStatusBadge({ status }) {
  ...
  return (
    <span className={`text-[10px] md:text-[11px] font-medium px-2 py-1 rounded-full ${styles[status]}`}>
      {status}
    </span>
  );
}
        ...
        {documents.map(doc => (
          ...
          className="flex items-center justify-between p-2 md:p-3 border border-slate-200 rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] md:text-[12px] font-semibold text-[#0B1E3D]">{doc.name}</p>
                <p className="text-[10px] md:text-[11px] text-gray-500">{doc.description}</p>
              </div>
            </div>
