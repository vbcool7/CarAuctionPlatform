...
function CancelledAuctionsList({ setCurrentPage, onSelectVehicle }) {

    const [activeTab, setActiveTab] = useState("all-cancelled");

    // tabs
    const tabs = [
        { id: 'all-cancelled', label: 'All Cancelled' },
                ...
                <table className="w-full text-left table-fixed">
                    <thead className="bg-gray-100 border-b border-gray-200 text-slate-500 uppercase text-[11px] font-bold tracking-wider">
                        <tr>
                            ...
                            <th className="px-6 py-4 w-40">Cancelled By</th>
                            <th className="px-6 py-4 w-25">Actions</th>
                        </tr>
                    </thead>

                    <tbody className="divide-y divide-amber-50">
                        {allAuctionData.filter((item) => item.status === "Cancelled").map((auction) => (
                            <tr
                                key={auction.id}
                                ...
                                {/* starting price */}
                                <td className='px-6 py-4'>
                                    <div className="font-bold text-green-600 text-sm">{auction.startingPrice}</div>
                                </td>
                                ...
                                {/* cancel date */}
                                <td className="px-6 py-4 text-[13px] text-slate-700 leading-tight">
                                    <div>{auction.cancelledDate}</div>
                                    <div className="font-semibold">{auction.cancelledTime}</div>
                                </td>
                                ...
                                {/* cancel by */}
                                <td className="px-6 py-4 text-sm font-normal text-gray-600 whitespace-nowrap">
                                    {auction.cancelledBy}
                                </td>
